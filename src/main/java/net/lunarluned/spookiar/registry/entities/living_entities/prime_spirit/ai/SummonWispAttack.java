...
public class SummonWispAttack extends Behavior<PrimeSpirit> {
	public SummonWispAttack() {
		super(ImmutableMap.of(MemoryModuleType.LOOK_TARGET, MemoryStatus.REGISTERED, MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT, ModMemoryModules.GHOST_SPAWNING_WISP_COOLDOWN, MemoryStatus.VALUE_ABSENT));
	}
	...
	protected boolean checkExtraStartConditions(ServerLevel serverLevel, PrimeSpirit primeSpirit) {
		...
		return BehaviorUtils.canSee(primeSpirit, livingEntity) && primeSpirit.closerThan(livingEntity, 10) && primeSpirit.getBrain().getMemory(ModMemoryModules.GHOST_SPAWNING_WISP_COOLDOWN).isEmpty() && primeSpirit.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).isPresent();
	}
			...
			return BehaviorBuilder.create((instance) -> {
				return instance.group(instance.registered(MemoryModuleType.LOOK_TARGET), instance.present(MemoryModuleType.ATTACK_TARGET), instance.absent(MemoryModuleType.ATTACK_COOLING_DOWN), instance.present(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES)).apply(instance, (memoryAccessor, memoryAccessor2, memoryAccessor3, memoryAccessor4) -> {
					return (world, mob, l) -> {
						...
						if (primeSpirit.getBrain().getMemory(ModMemoryModules.GHOST_SPAWNING_WISP_COOLDOWN).isEmpty()) {
						if (((NearestVisibleLivingEntities)instance.get(memoryAccessor4)).contains(livingEntity)) {
							...
							Wisp wisp;
							for (int w = 0; w < 5; w++){
							wisp = ModEntities.WISP.create(mob.level());

							wisp.setPos(primeSpirit.getX(), primeSpirit.getEyeY() + 0.2, primeSpirit.getZ());

							mob.level().addFreshEntity(wisp);
						}
							...
							primeSpirit.getBrain().setMemory(ModMemoryModules.PRIME_SPIRIT_STOMPING_COOLDOWN, UniformInt.of(300, 400).sample(primeSpirit.level().getRandom()));
